...
void Builtins::Generate_ResumeGeneratorTrampoline(MacroAssembler* masm) {
  ...
  __ LoadTaggedField(cp, FieldMemOperand(c5, JSFunction::kContextOffset));
  __ DebugAssertValidContext(cp);
  ...
  __ Ldr(dispatch_handle.W(),
         FieldMemOperand(c5, JSFunction::kDispatchHandleOffset));
  ...
  // Copy the function arguments from the generator object's register file.
  {
    ...
    __ Add(c6, c6, Operand(x10, LSL, kTaggedSizeLog2));
    __ Add(c6, c6, Operand(OFFSET_OF_DATA_START(FixedArray) - kHeapObjectTag));
    __ Bind(&loop);
    __ Sub(x10, x10, 1);
    __ LoadTaggedField(c11, MemOperand(c6, -kTaggedSize, PreIndex));
  ...
  __ Bind(&prepare_step_in_if_stepping);
  {
    ...
    __ LoadRoot(c6, RootIndex::kTheHoleValue);
    __ Push(c1, padregc, c5, c6);
...
void Generate_JSEntryVariant(MacroAssembler* masm, StackFrame::Type type,
  ...
  __ B(ne, &done);
  __ Str(cfp, __ AsMemOperand(IsolateFieldId::kJSEntrySP));

  __ Bind(&done);

  __ Ldr(c7, __ AsMemOperand(IsolateFieldId::kFastCCallCallerFP));
  __ Str(czr, __ AsMemOperand(IsolateFieldId::kFastCCallCallerFP));

  __ Ldr(c8, __ AsMemOperand(IsolateFieldId::kFastCCallCallerPC));
  __ Str(czr, __ AsMemOperand(IsolateFieldId::kFastCCallCallerPC));
  __ Push(c10, c11, c7, c8);
  ...
  // that.
  {
    ...
    __ Str(c0, __ AsMemOperand(IsolateFieldId::kException));
  }
  ...
  __ Ldr(c10, __ AsMemOperand(IsolateFieldId::kHandler));
  __ Push(padregc, c10);
  ...
  // Set this new handler as the current one.
  {
    ...
    __ Mov(scratch, csp);  // We can't store sp to memory location.
  ...
  __ Str(c10, __ AsMemOperand(IsolateFieldId::kHandler));
  ...
  __ Pop(c10, c11);
  __ Str(c10, __ AsMemOperand(IsolateFieldId::kFastCCallCallerPC));
  __ Str(c11, __ AsMemOperand(IsolateFieldId::kFastCCallCallerFP));
  ...
  Label non_outermost_js_2;
  {
    ...
    __ Str(czr, __ AsMemOperand(IsolateFieldId::kJSEntrySP));
...
void Builtins::Generate_BaselineOutOfLinePrologue(MacroAssembler* masm) {
  ...
  temps.Include(CPURegList(kCRegSizeInBits, {c12, c13, c14, c15}));

  auto descriptor =
      Builtins::CallInterfaceDescriptorFor(Builtin::kBaselineOutOfLinePrologue);
  Register closure = descriptor.GetRegisterParameter(
      BaselineOutOfLinePrologueDescriptor::kClosure);
  DCHECK_IMPLIES(V8_TARGET_CHERI_BOOL, closure.IsC());
  // Load the feedback cell and vector from the closure.
  Register feedback_cell = temps.AcquireC();
  ...
  FrameScope frame_scope(masm, StackFrame::MANUAL);
  {
    ...
    __ Push(argc.C(), bytecode_array, feedback_cell, feedback_vector);
    __ AssertFeedbackVector(feedback_vector, scratch.C());
  }
  ...
  __ bind(&call_stack_guard);
  {
    ...
    __ Push(maybe_dispatch_handle.C(), new_target);
    __ SmiTag(frame_size);
    __ PushArgument(frame_size);
    __ CallRuntime(Runtime::kStackGuardWithGap);
    __ Pop(new_target, maybe_dispatch_handle.C());
  }
  __ LoadRoot(kInterpreterAccumulatorRegister, RootIndex::kUndefinedValue);
  __ Ret();
}
...
void Builtins::Generate_InterpreterEntryTrampoline(
  ...
  __ LoadFeedbackVector(feedback_vector, closure, c7, &push_stack_frame);
...
void Builtins::Generate_ConstructForwardAllArgsImpl(
  ...
  // Load the frame pointer into c4.
  switch (which_frame) {
    case ForwardWhichFrame::kCurrentFrame:
      __ Move(c4, fp);
      break;
    case ForwardWhichFrame::kParentFrame:
      __ Ldr(c4, MemOperand(fp, StandardFrameConstants::kCallerFPOffset));
      break;
  }

  // Load the argument count into x0.
  __ Ldr(x0, MemOperand(c4, StandardFrameConstants::kArgCOffset));

  // Point c4 to the base of the argument list to forward, excluding the
  // receiver.
  __ Add(c4, c4,
  ...
  __ Claim(slots_to_claim);
  {
    ...
    Register scratch = temps.AcquireX();
    __ Sub(scratch, slots_to_claim, 1);
    __ Poke(padregc, Operand(scratch.C(), LSL, kSystemPointerSizeLog2));
  }

  // Copy the arguments.
  __ Sub(argc_without_receiver, x0, kJSArgcReceiverSlots);
  __ SlotAddress(stack_addr, 1);
  __ DebugAssertCapabilityIsTagged(c4);
#if V8_TARGET_CHERI
  __ CopyCapabilities(stack_addr, c4, argc_without_receiver);
#else
...
void NewImplicitReceiver(MacroAssembler* masm) {
  ...
  // Save live registers.
  __ SmiTag(x0);
  __ Push(c0, c1, c3, padregc);
  __ CallBuiltin(Builtin::kFastNewObject);
  // Save result.
  __ Mov(implicit_receiver, x0);
  // Restore live registers.
  __ Pop(padregc, c3, c1, c0);
  __ SmiUntag(x0);
...
void Builtins::Generate_InterpreterPushArgsThenFastConstructFunction(
  ...
  __ LoadRoot(c2, RootIndex::kTheHoleValue);
  __ Push(c2, padregc);

  // Push arguments + implicit receiver.
  GenerateInterpreterPushArgs(masm, x0, c4, Register::no_reg(),
                              ConvertReceiverMode::kNullOrUndefined,
                              InterpreterPushArgsMode::kOther);
  __ Poke(c2, 0 * kSystemPointerSize);
...
static void Generate_InterpreterEnterBytecode(MacroAssembler* masm) {
  ...
  __ LoadTrustedUnknownPointerField(
      c1, FieldMemOperand(c1, SharedFunctionInfo::kTrustedFunctionDataOffset),
      kInterpreterDispatchTableRegister,
      {{INTERPRETER_DATA_TYPE, &is_interpreter_data}});
  __ B(&builtin_trampoline);

  __ bind(&is_interpreter_data);
  __ LoadInterpreterDataInterpreterTrampoline(c1, c1);
  __ LoadCodeInstructionStart(c1, c1, kJSEntrypointTag);
...
void OnStackReplacement(MacroAssembler* masm, OsrSourceTier source,
  ...
  DCHECK_IMPLIES(V8_TARGET_CHERI_BOOL, maybe_target_code.IsC());
  ...
  {
    FrameScope scope(masm, StackFrame::INTERNAL);
    __ Push(expected_param_count, padregc);
    __ CallRuntime(Runtime::kCompileOptimizedOSR);
    DCHECK_EQ(maybe_target_code, c0);
    __ Pop(padregc, expected_param_count);
  }
  ...
  // OSR entry tracing.
  {
    ...
    __ Ldrsb(scratch.X(), MemOperand(scratch));
    __ Tst(scratch.X(), 0xFF);  // Mask to the LSB.
    __ B(eq, &next);

    {
      FrameScope scope(masm, StackFrame::INTERNAL);
      // Preserve arguments.
      __ Push(maybe_target_code, expected_param_count.C());
      __ CallRuntime(Runtime::kLogOrTraceOptimizedOSREntry, 0);
      __ Pop(expected_param_count.C(), maybe_target_code);
    }
...
void Builtins::Generate_MaglevFunctionEntryStackCheck(MacroAssembler* masm,
  ...
  ASM_CODE_COMMENT(masm);
  {
    ...
    if (save_new_target) {
      ...
      __ Push(kJavaScriptCallNewTargetRegister, padregc);
    }
    __ PushArgument(c0);
    __ CallRuntime(Runtime::kStackGuardWithGap, 1);
    if (save_new_target) {
      __ Pop(padregc, kJavaScriptCallNewTargetRegister);
    }
  }
  __ Ret();
}
...
static void GenerateCall(MacroAssembler* masm, Register argc, Register target,
  ...
  __ Bind(&non_callable);
  {
    ...
    if (!error_string_root.has_value()) {
      ...
      __ LoadRoot(c2, error_string_root.value());
      __ Push(target, c2);
...
void Generate_PrepareForCopyingVarargs(MacroAssembler* masm, Register argc,
  ...
  // call.
  {
    ...
    __ Tbz(len, 0, &exit);
    __ Str(czr, MemOperand(csp, len, LSL, kSystemPointerSizeLog2));
  }
  __ Bind(&exit);
}
...
void Builtins::Generate_CallFunction(MacroAssembler* masm,
  ...
  __ InvokeFunctionCode(c1, no_reg, x0, InvokeType::kJump);
}
...
void Builtins::Generate_CallBoundFunctionImpl(MacroAssembler* masm) {
  ...
  __ Poke(c10, __ ReceiverOperand());
...
void Builtins::Generate_WasmLiftoffFrameSetup(MacroAssembler* masm) {
  ...
  __ StoreTaggedField(
      scratch.C(), FieldMemOperand(vector, OFFSET_OF_DATA_START(FixedArray)));
  __ Push(vector, czr);
  __ Ret();
  ...
  __ Push(c0, kWasmImplicitArgRegister, func_index.C(), c1);
  __ Mov(cp, Smi::zero());
...
void Builtins::Generate_WasmCompileLazy(MacroAssembler* masm) {
  ...
  temps.Exclude(x17);
  {
    ...
    __ Push(kWasmImplicitArgRegister, kWasmCompileLazyFuncIndexRegister.C());
    ...
    __ SmiUntag(x17, kReturnRegister0.X());
...
void SwitchStackPointerAndSimulatorStackLimit(MacroAssembler* masm,
  ...
  DCHECK_IMPLIES(V8_TARGET_CHERI_BOOL, tmp.IsC());
  DCHECK_IMPLIES(V8_TARGET_CHERI_BOOL, stack.IsC());
  if (masm->options().enable_simulator_code) {
    UseScratchRegisterScope temps(masm);
    DCHECK_NE(c16, tmp);
    temps.Exclude(c16);
    __ Ldr(tmp, MemOperand(stack, wasm::kStackSpOffset));
    __ Ldr(c16, MemOperand(stack, wasm::kStackLimitOffset));
    __ Mov(csp, tmp);
...
void LoadTargetJumpBuffer(MacroAssembler* masm, Register target_stack,
                          Register tmp) {
  __ Str(czr, MemOperand(fp, WasmJspiFrameConstants::kGCScanSlotCountOffset));
  DCHECK_IMPLIES(V8_TARGET_CHERI_BOOL, target_stack.IsC());
  DCHECK_IMPLIES(V8_TARGET_CHERI_BOOL, tmp.IsC());
  // Switch stack!
...
void ReloadParentStack(MacroAssembler* masm, Register return_reg,
  ...
  DCHECK_IMPLIES(V8_TARGET_CHERI_BOOL, return_reg.IsC());
  DCHECK_IMPLIES(V8_TARGET_CHERI_BOOL, return_value.IsC());
  DCHECK_IMPLIES(V8_TARGET_CHERI_BOOL, context.IsC());
  DCHECK_IMPLIES(V8_TARGET_CHERI_BOOL, parent.IsC());
  // Switch stack!
...
void RestoreParentSuspender(MacroAssembler* masm, Register tmp1) {
  DCHECK_IMPLIES(V8_TARGET_CHERI_BOOL, tmp1.IsC());
...
void Builtins::Generate_WasmFXSuspend(MacroAssembler* masm) {
  ...
  DCHECK(!AreAliased(c4, arg_buffer, target_stack));
  LoadJumpBuffer(masm, target_stack, true, c4);
  __ Trap();
  __ Bind(&resume, BranchTargetIdentifier::kBtiJump);
...
void Builtins::Generate_WasmFXReturn(MacroAssembler* masm) {
  ...
  LoadJumpBuffer(masm, parent, true, c3);
  __ Trap();
}
...
void SwitchToAllocatedStack(MacroAssembler* masm, RegisterAllocator& regs,
  ...
  __ LoadExternalPointerField(
      ...
      kWasmStackMemoryTag);
  DCHECK_IMPLIES(V8_TARGET_CHERI_BOOL, wasm_instance.IsC());
  DCHECK_IMPLIES(V8_TARGET_CHERI_BOOL, wrapper_buffer.IsC());
...
void SwitchBackAndReturnPromise(MacroAssembler* masm, RegisterAllocator& regs,
  ...
  ReloadParentStack(masm, promise, return_value, kContextRegister, tmp.C(), tmp2,
                    tmp3);
  ...
  if (mode == wasm::kPromise) {
    ...
    __ Push(padregc, promise);
    __ CallBuiltin(Builtin::kFulfillPromise);
    __ Pop(promise, padregc);
  }
  FREE_REG(promise);
...
void JSToWasmWrapperHelper(MacroAssembler* masm, wasm::Promise mode) {
  ...
  {
    DEFINE_SCOPED_C(params_start);
    __ Ldr(params_start,
           MemOperand(wrapper_buffer,
                      JSToWasmWrapperFrameConstants::kWrapperBufferParamStart));
    {
      // Push stack parameters on the stack.
      DEFINE_SCOPED_C(params_end);
      __ Ldr(params_end,
             MemOperand(wrapper_buffer,
                        JSToWasmWrapperFrameConstants::kWrapperBufferParamEnd));
      DEFINE_SCOPED_C(last_stack_param);
      ...
      // Push parameter
      {
        DEFINE_SCOPED_C(scratch1);
        DEFINE_SCOPED_C(scratch2);
  ...
  ASSIGN_PINNED(wrapper_buffer, c2);
  ...
  if (stack_switch) {
    __ Ldr(c1, MemOperand(fp, WasmJspiFrameConstants::kResultArrayOffset));
    __ Ldr(c0, MemOperand(fp, WasmJspiFrameConstants::kImplicitArgOffset));
  } else {
    __ Ldr(c1, MemOperand(
                   fp, JSToWasmWrapperFrameConstants::kResultArrayParamOffset));
    __ Ldr(c0,
           MemOperand(fp, JSToWasmWrapperFrameConstants::kImplicitArgOffset));
  }
  Register scratch = c3;
  GetContextFromImplicitArg(masm, c0, scratch);
...
void SwitchSimulatorStackLimit(MacroAssembler* masm) {
  if (masm->options().enable_simulator_code) {
    UseScratchRegisterScope temps(masm);
    temps.Exclude(c16);
    __ LoadStackLimit(c16, StackLimitKind::kRealStackLimit);
    __ hlt(kImmExceptionIsSwitchStackLimit);
  }
}

static constexpr Register kOldSPRegister = c23;
...
void SwitchToTheCentralStackIfNeeded(MacroAssembler* masm, Register argc_input,
  ...
  static constexpr Register central_stack_sp = c4;
  DCHECK(!AreAliased(central_stack_sp, argc_input, argv_input, target_input));
  {
    __ Push(argc_input.C(), target_input, argv_input, padregc);
    ...
    __ Pop(padregc, argv_input, target_input, argc_input.C());
  }
  {
    ...
    UseScratchRegisterScope temps{masm};
    Register new_sp_after_call = temps.AcquireC();
  ...
  __ Sub(csp, central_stack_sp, kReturnAddressSlotOffset + kPadding);
...
void Builtins::Generate_CEntry(MacroAssembler* masm, int result_size,
  ...
  UseScratchRegisterScope temps(masm);
  temps.Exclude(c17);
  __ Ldr(c17, __ AsMemOperand(IsolateFieldId::kPendingHandlerEntrypoint));
  __ PrepareC64Jump(c17);
  __ Br(c17);
}
...
void Builtins::Generate_WasmHandleStackOverflow(MacroAssembler* masm) {
  ...
  {
    UseScratchRegisterScope temps(masm);
    Register new_fp = temps.AcquireC();
...
void Builtins::Generate_CallApiCallbackImpl(MacroAssembler* masm,
  ...
  if (v8_flags.debug_code) {
    ...
    __ Add(scratch, csp, kArgcOffsetFromSP + kPCSlotSize - FC::kFCIArgcOffset);
    __ cmp(scratch, fp);
...
void Builtins::Generate_DirectCEntry(MacroAssembler* masm) {
  ...
  __ Call(c10);                             // Call the C++ function.
...
void Generate_DeoptimizationEntry(MacroAssembler* masm,
  ...
  const int kSimd128RegistersOffset = saved_registers.Count() * kCRegSize;
  ...
  //  - x3: fp-to-sp delta
  __ Mov(c4, ExternalReference::isolate_address());
  ...
  CopyRegListToFrame(masm, c1, FrameDescription::simd128_registers_offset(),
...
void Builtins::Generate_InterpreterOnStackReplacement_ToBaseline(
  ...
  Register feedback_cell = c2;
  Register feedback_vector = c15;
  __ LoadTaggedField(feedback_cell,
                     FieldMemOperand(closure, JSFunction::kFeedbackCellOffset));
  __ LoadTaggedField(
      feedback_vector,
  ...
  __ Push(padregc, kInterpreterAccumulatorRegister);
  {
    ...
    __ Mov(kCArgRegs[1].X(), kInterpreterBytecodeOffsetRegister);
...
void Builtins::Generate_RestartFrameTrampoline(MacroAssembler* masm) {
  ...
  __ InvokeFunction(c1, x0, InvokeType::kJump,
