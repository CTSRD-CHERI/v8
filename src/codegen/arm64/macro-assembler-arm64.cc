...
void MacroAssembler::GenerateTailCallToReturnedCode(
  ...
  // -----------------------------------
  {
    ...
    Register maybe_dispatch_handle = V8_JS_LINKAGE_INCLUDES_DISPATCH_HANDLE_BOOL
                                         ...
                                         : padregc;
    DCHECK_IMPLIES(V8_TARGET_CHERI_BOOL, maybe_dispatch_handle.IsC());
    ...
    Push(kJavaScriptCallTargetRegister, kJavaScriptCallNewTargetRegister,
         kJavaScriptCallArgCountRegister.C(), maybe_dispatch_handle);
    ...
    // handle.
    DCHECK_IMPLIES(V8_TARGET_CHERI_BOOL,
                   kJavaScriptCallNewTargetRegister.IsC() &&
                       kJavaScriptCallTargetRegister.IsC());
    Pop(maybe_dispatch_handle, kJavaScriptCallArgCountRegister.C(),
  ...
  LoadEntrypointFromJSDispatchTable(c2, kJavaScriptCallDispatchHandleRegister,
                                    c5);
  Move(c17, c2);
  Jump(c17);
}
...
int MacroAssembler::CallCFunction(Register function, int num_of_reg_args,
  ...
  if (set_isolate_data_slots == SetIsolateDataSlots::kYes) {
    ...
    UseScratchRegisterScope temps(this);
    Register pc_scratch = temps.AcquireC();
  ...
  if (set_isolate_data_slots == SetIsolateDataSlots::kYes) {
    ...
    Str(czr, ExternalReferenceAsOperand(IsolateFieldId::kFastCCallCallerFP));
  }
...
void MacroAssembler::JumpCodeObject(Register code_object, CodeEntrypointTag tag,
  ...
  DCHECK_IMPLIES(V8_TARGET_CHERI_BOOL, code_object.IsC());
  LoadCodeInstructionStart(code_object, code_object, tag);
...
void MacroAssembler::InvokePrologue(Register formal_parameter_count,
  ...
  Bind(&skip_move);
  Register pointer_next_value = c6;

  // Copy extra arguments as undefined values.
  {
    Label loop;
    Register undefined_value = c7;
    Register count = c8;
...
void MacroAssembler::CallDebugOnFunctionCall(Register fun, Register new_target,
  ...
  Pop(fun, new_target, actual_parameter_count.C(), dispatch_handle);
...
void MacroAssembler::CheckPageFlag(const Register& object, int mask,
  ...
  UseScratchRegisterScope temps(this);
  Register scratch = temps.AcquireX();
  And(scratch.C(), object, ~MemoryChunk::GetAlignmentMaskForAssembler());
  Ldr(scratch.X(), MemOperand(scratch.C(), MemoryChunk::FlagsOffset()));
  if (cc == ne) {
...
void MacroAssembler::LoadExternalPointerField(Register destination,
  ...
  Ldr(destination.W(), field_operand);
  Mov(destination.X(),
      Operand(destination.X(), LSR, kExternalPointerIndexShift));
...
void MacroAssembler::RecordWrite(Register object, Operand offset,
  ...
  if (v8_flags.slow_debug_code) Mov(slot_address.X(), Operand(kZapValue));
